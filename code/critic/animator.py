...
from visualize.curve_cubic_spline import *
import imageio



def name_init(full_name):
    words = full_name.split()
    initials = ''.join(word[0].upper() for word in words)
    return initials

class Animator:
    def __init__(self, vlz: Visualizer):
        self.vlz = vlz

        self.alphas = [.1, .1, .4, 1]
        self.histories = None
        self.label_off = None
        self.label_ball = None
        self.predict_off = None
        self.predict_ball = None
        self.show_name = True
    def plot_history(self, history_find, alpha=1):
        history_off = history_find.get('df_offense')
        history_def = history_find.get('df_defense')
        history_ball = history_find.get('df_ball')
        vals = fit_cs_curve_df(history_off)
        self.vlz.plot_curves(vals, part='off', alpha=alpha)
        self.vlz.plot_nodes(x=vals[:, 0, 0], y=vals[:, 1, 0], part='off', alpha=alpha)
        vals = fit_cs_curve_df(history_def)
        self.vlz.plot_curves(vals, part='def', alpha=alpha)
        self.vlz.plot_nodes(x=vals[:, 0, 0], y=vals[:, 1, 0], part='def', alpha=alpha)
        vals = fit_cs_curve_df(history_ball)
        self.vlz.plot_curves(vals, part='ball', alpha=alpha)
        self.vlz.plot_nodes(x=vals[:, 0, 0], y=vals[:, 1, 0], part='ball', alpha=alpha)

    def update_data(self, data):
        prediction = data.get('predict', None)
        label = data.get('label', None)
        prompt = data.get('prompt', None)

        history = prompt.split('"pitch_length_x": 105, "pitch_width_y": 68}')[-1].split("Let's think")[0].strip()
        history1, history2, history3 = json.loads(history)
        self.histories = [json_find(json.dumps(history1)), json_find(json.dumps(history2)), json_find(json.dumps(history3))]
        self.label_off = json_find(label)['df_offense'] if label else None
        self.label_ball = json_find(label)['df_ball'] if label else None
        self.predict_off = json_find(prediction)['df_offense'] if prediction else None
        self.predict_ball = json_find(prediction)['df_ball'] if prediction else None

    def animate_histories(self, n):
        # 初始化球场
        self.vlz.__init__()

        # 绘制历史
        alpha = self.alphas[-n:]
        for i, history in enumerate(self.histories[:n]):
            self.plot_history(history, alpha[i])

        # name
        if self.show_name:
            last_one = self.histories[n-1]
            history_off = last_one.get('df_offense')
            history_def = last_one.get('df_defense')
            self.vlz.plot_names(x=history_off['posX.c'].tolist(), y=history_off['posY.c'].tolist(), name=[name_init(n) for n in history_off['player'].tolist()])
            self.vlz.plot_names(x=history_def['posX.c'].tolist(), y=history_def['posY.c'].tolist(), name=[name_init(n) for n in history_def['player'].tolist()])

        # 展示
        # self.vlz.show()
        return self.vlz.save_to_PILImage()
    ...
    def animate_label(self):
        ...
        # 绘制label
        vals = fit_cs_curve_df(self.label_off)
        self.vlz.plot_curves(vals, part='off')
        self.vlz.plot_nodes(x=vals[:, 0, 0], y=vals[:, 1, 0], part='off', alpha=self.alphas[-1])

        vals = fit_cs_curve_df(self.label_ball)
        self.vlz.plot_curves(vals, part='ball')
        self.vlz.plot_nodes(x=vals[:, 0, 0], y=vals[:, 1, 0], part='ball', alpha=self.alphas[-1])

        if self.show_name:
            last_one = self.label_off
            self.vlz.plot_names(x=last_one['posX.c'].tolist(), y=last_one['posY.c'].tolist(), name=[name_init(n) for n in last_one['player'].tolist()])

        # 展示
        self.vlz.show()
        # return self.vlz.save_to_PILImage()

    def animate_predict(self, prediction=None):
        # 初始化球场
        self.vlz.__init__()
        ...
        # 手动插入prediction，用于多轮反事实
        if prediction is not None:
            predict_off = json_find(prediction)['df_offense']
            predict_ball = json_find(prediction)['df_ball']
        else:
            predict_off = self.predict_off
            predict_ball = self.predict_ball

        # 绘制prediction
        vals = fit_cs_curve_df(predict_off)
        self.vlz.plot_curves(vals, part='off_pred')
        self.vlz.plot_nodes(x=vals[:, 0, 0], y=vals[:, 1, 0], part='off_pred', alpha=self.alphas[-1])

        vals = fit_cs_curve_df(predict_ball)
        self.vlz.plot_curves(vals, part='ball_pred')
        self.vlz.plot_nodes(x=vals[:, 0, 0], y=vals[:, 1, 0], part='ball_pred', alpha=self.alphas[-1])

        if self.show_name:
            last_one = predict_off
            self.vlz.plot_names(x=last_one['posX.c'].tolist(), y=last_one['posY.c'].tolist(), name=[name_init(n) for n in last_one['player'].tolist()])
    ...
    def animate_predict_and_label(self, prediction=None):
        # 初始化球场
        self.vlz.__init__()
...
class Animator_dedicated:
    def __init__(self, vlz, save2PIL=True):
        self.vlz = vlz
        self.alpha_his = 0.15
        self.alpha_now = 1
        self.histories = None
        ...
        self.predict_ball = None
        self.show_name = True
        self.save2PIL = save2PIL

    def process_data(self, data):
        if len(data) == 1:
            data = data[0]
        vals = np.array(data)  # (T, N, 2)

        # 对每个点的轨迹在时间轴上进行平滑，保持起始和终止点不变
        def smooth_trajectory(vals, window=3):
            T, N, D = vals.shape
            smoothed = vals.copy()
            for n in range(N):
                for d in range(D):
                    ...
                    for t in range(1, T-1):
                        ...
                        right = min(T, t + window//2 + 1)
                        smoothed[t, n, d] = np.mean(vals[left:right, n, d])
                    # 起始和终止点保持原样
                    smoothed[0, n, d] = vals[0, n, d]
                    smoothed[T-1, n, d] = vals[T-1, n, d]
            return smoothed

        vals = fit_cs_curve_ar(vals)  # (T_密集, N, 2)
        vals = smooth_trajectory(vals, window=3)
        vals[:,:,0]+= 52.5  # x
        vals[:,:,1]+= 34  # y
        return vals

    def plot_data(self, vals, alpha=1, is_gt=True, mask_defend=True, mask_unrelevant=False, draw_magnifier=False, no_nodes=False):
        if draw_magnifier:
            relevant_players = self._get_relevant_players(vals, threshold=20)
            self._get_magnifier_window(vals, relevant_players, margin=1)

        # Curves
        self.vlz.plot_curves(vals[:,0:1,:], part='ball' if is_gt else 'ball_pred', alpha=alpha, zorder=10)
        if not mask_unrelevant:
            self.vlz.plot_curves(vals[:, 1:12, :], part='off' if is_gt else 'off_pred', alpha=alpha, zorder=9)
        else:
            self.vlz.plot_curves(vals[:, self._get_relevant_players(vals, threshold=20),:], part='off' if is_gt else 'off_pred', alpha=alpha, zorder=9)
        if not mask_defend:
            self.vlz.plot_curves(vals[:, 12:23, :], part='def' if is_gt else 'def_pred', alpha=alpha)
        else:
            self.vlz.plot_curves(vals[:, 12:23, :], part='def' if is_gt else 'def_pred', alpha=0.3)

        # Nodes
        if not no_nodes:
            self.vlz.plot_nodes(x=vals[-1, 0:1, 0], y=vals[-1, 0:1, 1], part='ball' if is_gt else 'ball_pred', alpha=alpha, zorder=10)
            if not mask_unrelevant:
                self.vlz.plot_nodes(x=vals[-1, 1:12, 0], y=vals[-1, 1:12, 1], part='off' if is_gt else 'off_pred', alpha=alpha, zorder=9)
            else:
                self.vlz.plot_nodes(x=vals[-1, self._get_relevant_players(vals, threshold=20), 0], y=vals[-1, self._get_relevant_players(vals, threshold=20), 1], part='off' if is_gt else 'off_pred', alpha=alpha, zorder=9)
            if not mask_defend:
                self.vlz.plot_nodes(x=vals[-1, 12:23, 0], y=vals[-1, 12:23, 1], part='def' if is_gt else 'def_pred', alpha=alpha)
            else:
                self.vlz.plot_nodes(x=vals[-1, 12:23, 0], y=vals[-1, 12:23, 1], part='def' if is_gt else 'def_pred', alpha=0.3)


    def animate_x(self, x, path='try.jpg'):
        self.vlz.__init__()
        x = np.array(x)[...,:2]
        self.plot_data(self.process_data(x), self.alpha_now)
        if not self.save2PIL:
            self.vlz.save(path)
        else:
            return self.vlz.save_to_PILImage()

    def animate_label(self, x, label, mask_defend=False, path='try.jpg'):
        self.vlz.__init__()
        self.plot_data(self.process_data(x), self.alpha_his)
        self.plot_data(self.process_data(label), self.alpha_now, mask_defend=False)
        if not self.save2PIL:
            self.vlz.save(path)
        else:
            return self.vlz.save_to_PILImage()

    def animate_predict(self, x, prediction, mask_defend=False, mask_unrelevant=False, draw_magnifier=False, path='try.jpg'):
        # 初始化球场
        self.vlz.__init__()
        self.plot_data(self.process_data(x), self.alpha_his)
        self.plot_data(self.process_data(prediction), self.alpha_now, is_gt=False, mask_defend=mask_defend, mask_unrelevant=mask_unrelevant, draw_magnifier=draw_magnifier)
        # self.vlz.show()
        if not self.save2PIL:
            self.vlz.save(path)
        else:
    ...
    def animate_predict_forQ(self, x=None, prediction=None, carrier_role=None, recipient_role=None, mask_defend=False, mask_unrelevant=False, draw_magnifier=False, path='try.jpg', name_list=None, role_list=None):
        # 初始化球场
        self.vlz.__init__()
        prediction = np.array(prediction)[...,:2]
        if x is not None:
            x = np.array(x)[...,:2]
            his_pre = np.concatenate((x[-1:, :, :], prediction), axis=0)
        else:
            his_pre = prediction
        self.plot_data(self.process_data(his_pre), self.alpha_now, is_gt=True, mask_defend=mask_defend, mask_unrelevant=mask_unrelevant, draw_magnifier=draw_magnifier)

        if role_list is not None and name_list is not None:
            self.vlz.plot_names(self.process_data(his_pre)[-1, 1:12, 0], self.process_data(his_pre)[-1, 1:12, 1], name_list[1:12], role_list[1:12], initials=True)
            if carrier_role is not None:
                carrier_id = role_list.index(carrier_role)
                self.vlz.ax.plot(
                    self.process_data(his_pre)[-1, carrier_id, 0],
                    self.process_data(his_pre)[-1, carrier_id, 1],
                    ms=11,
                    ...
                    markeredgewidth=1.5,
                    linestyle='None',
                    alpha=self.alpha_now, zorder=9)
            if recipient_role is not None:
                recipient_id = role_list.index(recipient_role)
                self.vlz.ax.plot(
                    self.process_data(his_pre)[-1, recipient_id, 0],
                    self.process_data(his_pre)[-1, recipient_id, 1],
                    ms=11,
                    markerfacecolor='yellow',
                    marker='X',
                    markeredgecolor='none',
    ...
    def video_predict_forQ(self, x=None, prediction=None, carrier_role=None, recipient_role=None, mask_defend=False, mask_unrelevant=False, draw_magnifier=False, path='try.mp4', name_list=None, role_list=None):
        prediction = np.array(prediction)[...,:2]
        if x is not None:
            x = np.array(x)[...,:2]
            his_pre = np.concatenate((x[-1:, :, :], prediction), axis=0)
        ...
        his_pre = self.process_data(his_pre)
        frames = []
        for t in range(1, his_pre.shape[0]):
            self.vlz.__init__()
            self.plot_data(his_pre[:t, :, :], self.alpha_now, is_gt=True, mask_defend=mask_defend, mask_unrelevant=mask_unrelevant, draw_magnifier=draw_magnifier)
            if role_list is not None and name_list is not None:
                self.vlz.plot_names(his_pre[t, 1:12, 0], his_pre[t, 1:12, 1], name_list[1:12], role_list[1:12], initials=True)
                if carrier_role is not None:
                    carrier_id = role_list.index(carrier_role)
                    self.vlz.ax.plot(
                        his_pre[t, carrier_id, 0],
                        his_pre[t, carrier_id, 1],
                        ms=11,
                        markerfacecolor='yellow',
                        marker='h',
                        markeredgecolor='none',
                ...
                if recipient_role is not None:
                    recipient_id = role_list.index(recipient_role)
                    self.vlz.ax.plot(
                        his_pre[t, recipient_id, 0],
                        his_pre[t, recipient_id, 1],
                        ms=11,
                        markerfacecolor='yellow',
                        marker='X',
                        markeredgecolor='none',
                        ...
                        linestyle='None',
                        alpha=self.alpha_now, zorder=9)
            img = self.vlz.save_to_PILImage()
            frames.append(np.array(img))
        imageio.mimsave(path, frames, fps=20, format='ffmpeg', macro_block_size=None)


    def animate_predict_forQ_defenseLabel(self, x, prediction, path='try.jpg'):
        self.vlz.__init__()
        x, prediction = np.array(x), np.array(prediction)
        self.plot_data(self.process_data(x[:, -16:, :, :]), self.alpha_his, no_nodes=True)  # part of history
        his_pre = np.concatenate((x[:, -1:, :, :], prediction), axis=1)
        self.plot_data(self.process_data(his_pre), self.alpha_now, is_gt=True, mask_defend=False, mask_unrelevant=False, draw_magnifier=False)
        # self.vlz.show()
        if self.save2PIL:
            return self.vlz.save_to_PILImage()
        else:
    ...
    def animate_predict_and_label(self, x, prediction, label, path='try.jpg'):
        self.vlz.__init__()
        self.plot_data(self.process_data(x), self.alpha_his)
        self.plot_data(self.process_data(label), self.alpha_now, is_gt=True)
        self.plot_data(self.process_data(prediction), self.alpha_now, is_his=False)
        if not self.save2PIL:
            self.vlz.save(path)
        else:
            return self.vlz.save_to_PILImage()

    def video_x(self, x, path='try.mp4'):
        frames = []
        x_np = np.array(x)
        x_np = self.process_data(x_np)
        T = x_np.shape[0]
        for t in range(1, T):
            self.vlz.__init__()
            self.plot_data(x_np[:t+1, :, :], self.alpha_now)
            img = self.vlz.save_to_PILImage()
            frames.append(np.array(img))
        imageio.mimsave(path, frames, fps=10, format='ffmpeg', macro_block_size=None)

    def video_predict_forQ_defenseLabel(self, x, prediction, label, path='try.mp4'):
        frames = []
        x = np.array(x)
        prediction = np.array(prediction)
        label = np.array(label)
        # prediction_dL = np.concatenate((prediction[:, :, :12, :], label[:, :, 12:, :]), axis=2)  # 防守球员换为label的
        prediction = np.concatenate((x[:, -1:, :, :], prediction), axis=1)

        x = self.process_data(x)
        prediction = self.process_data(prediction)
        label = self.process_data(label)
        T = prediction.shape[0]
        for t in range(T):
            self.vlz.__init__()
            self.plot_data(x[-30:, :, :], self.alpha_his, no_nodes=True)  # history last 3 second
            self.plot_data(prediction[:t+1, :, :])
            img = self.vlz.save_to_PILImage()
            frames.append(np.array(img))
        imageio.mimsave(path, frames, fps=10, format='ffmpeg', macro_block_size=None)

    def _get_relevant_players(self, data, threshold=20):
        """
        返回所有时刻中距离球小于threshold的球员索引集合。
        data: (T, N, 2) ndarray, 0号为球，1-11为off, 12-22为def
        返回: 相关球员的索引list（包括球本身0）
        """
        vals = np.array(data)  # (T, N, 2)
        ball_traj = vals[:, 0, :]  # (T, 2)
        relevant = set([])
        for n in range(1, 12):  # attack players
            dists = np.linalg.norm(vals[:, n, :] - ball_traj, axis=1)
            if np.any(dists < threshold):
                relevant.add(n)
        return sorted(list(relevant))

    def _get_magnifier_window(self, vals, relevant_idx, margin=1, scale=1.2):
        """
        计算放大镜窗口，覆盖球轨迹和所有相关球员轨迹，矩形区域，以所有相关点的边界为基础，margin为边界外扩像素，scale为放大比例。
        返回(left, right, bottom, top)
        """
        relevant_traj = vals[:, relevant_idx, :].reshape(-1, 2)
        min_x, min_y = relevant_traj.min(axis=0)
        max_x, max_y = relevant_traj.max(axis=0)
        ...
        self.vlz.draw_magnifier(left, right, bottom, top)
...
if __name__ == '__main__':
    vlz = Visualizer()
    animator = Animator_dedicated(vlz, save2PIL=False)
    ...
    # X = np.array(X).transpose(0, 1, 3, 2).tolist()
    # Y = np.array(Y).transpose(0, 1, 3, 2).tolist()
    # # animator.animate_x(X)
    # animator.animate_predict(X, Y, mask_unrelevant=True, mask_defend=True)

    # animator = Animator_dedicated(vlz, save2PIL=True)
    # animator.video_predict_forQ_defenseLabel(X, Y, Y, path='try_2.mp4')
    # animator.video_x(X)
    ...
    with open('/home/trl/fllm/ma_reflection/givego_curated_0730_dist_thresh_2_with_player.jsonl', 'r') as file:
        ...
        for index, line in enumerate(file):
            if index == 2:
                ...
                import json
                animator.animate_predict_forQ(history[0],
                                              label[0],
                                              carrier_role=json.loads(condition)['carrier_role'],
                                              recipient_role=json.loads(condition)['recipient_role'],
                                              name_list=description['name_list'],
                                              role_list=description['role_list'],
                                              path='try_2.pdf')
                animator.video_predict_forQ(history[0],
                                            label[0],
                                            carrier_role=json.loads(condition)['carrier_role'],
                                            recipient_role=json.loads(condition)['recipient_role'],
                                            name_list=description['name_list'],
