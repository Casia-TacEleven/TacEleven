...
class CaleyXGCalculator(XGCalculator):
    ...
    def _beta(dribble_goal, direct_free_kick, cross, header):
        if dribble_goal:
            ...
            return {'1': -3.19, 'distance': -0.095, 'inverse_distance': 3.18, 'angle': 1.88,
                    ...
                    'game_state_sgn': 0.03, 'Bundesliga': 0.07, 'EPL': 0.05, 'LaLiga': 0.05, 'SerieA': -0.07, 'our_league': 1}
    ...
    def eval(self, **kwargs):
        ...
        shot_params = {k: (float(v) if not isinstance(v, tuple) else v) for k, v in kwargs.items()}
        if cross_or_header:
...
class XG:
    def __init__(self):
        ...
        self.pitchWidthY = 68
        self.xg_calculator = CaleyXGCalculator()

    def get_metrics(self, trajectory: list):
        assert len(trajectory) > 1, len(trajectory)
        trajectory = np.array(trajectory)
        trajectory[..., 0] = self.pitchLengthX/2 - trajectory[..., 0]
        trajectory[..., 1] = self.pitchWidthY/2 - trajectory[..., 1]
        ...
        ball_x_assist, ball_y_assist = trajectory_ball[0, 0, 0], trajectory_ball[0, 0, 1]
        for t in range(time):
            ball_x, ball_y = trajectory_ball[t, 0, 0], trajectory_ball[t, 0, 1]
            score = self.xg_calculator.eval(x=ball_y, y=ball_x, assist_x=ball_y_assist, assist_y=ball_x_assist, our_league=True)  # !! xg_calculator中x与y恰好相反
            metrics['ball']['raw'].append(score)

        metrics['ball']['average'], metrics['ball']['start'], metrics['ball']['end'] = np.mean(metrics['ball']['raw']), metrics['ball']['raw'][0], metrics['ball']['raw'][-1]
        return metrics

def draw_binStatistic(statistic, pitch=None, is_flipud=True, title='bin statistic', is_save=True, save_path=None):
    if pitch is None:
    ...
    # 创建一个虚拟的bin_statistic结果来获取正确的格式
    # bins=(32, 20) 意味着网格是 33x21，数据应该是 (20, 32)
    temp = pitch.bin_statistic([], [], statistic='count', bins=(32, 20), normalize=False)

    # 确保输入数据的维度是 (20, 32) 来匹配 bins=(32, 20)
    if statistic.shape == (32, 20):
        statistic = statistic.T  # 转置为正确维度
    ...
    if not is_save:
        ...
        plt.savefig(f'xg_map.pdf' if save_path is None else save_path)
...
if __name__ == '__main__':
    ...
    pitch = Pitch(
        line_color='black', pitch_type='custom',
        pitch_length=105,
        pitch_width=68,
        line_zorder=2
    )
    # 根据bins=(32, 20)，数据应该是(20, 32)的格式
    # 因为bins定义的是x和y方向的bin数量，对应的数据维度应该是(y_bins, x_bins)
    x_values = np.linspace(0, 105, 32)
    y_values = np.linspace(0, 68, 20)
    heatmap = np.zeros((len(x_values), len(y_values)))  # (32, 20)

    for i, x in enumerate(x_values):
        for j, y in enumerate(y_values):
            heatmap[i, j] = xg.eval(x=(68-y), y=(105-x), our_league=True)

    xg_map = heatmap.copy()
    draw_binStatistic(xg_map, pitch)
