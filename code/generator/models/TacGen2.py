...
class TacGen2(nn.Module):
    ...
    Args:
        ...
        num_pre (int): Number of prediction steps.
        use_bias (bool): Whether to use bias in Fully Connected layers.

    Input types:
        * **X** (PyTorch Float Tensor) - Input sequence, with shape (batch_size, num_hist, num of nodes, 2).
        * **SE** (Pytorch Float Tensor) - Spatial embedding, with shape (numbed of nodes, K * d).
        * **TE** (Pytorch Float Tensor) - Temporal embedding, with shape (batch_size, num_his + num_pred, 3).
        * **LE** (Pytorch Float Tensor) - Language embedding, with shape (batch_size, 768).
    ...
    def forward(
        ...
        for net in self._decoder_layers:
            X_tgt = net(H, X_tgt, TE_tgt, tgt_key_padding_mask)
