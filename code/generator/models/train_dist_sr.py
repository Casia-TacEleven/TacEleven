...
def train_dist(model, args, loss_criterion, optimizer, scheduler):
    ...
    train_dataloader = DataLoader(train_dataset,
                                  ...
                                  collate_fn=lambda batch: collate_fn_self_regression(batch, language_embedding, args.num_pred, remove_playerName=True))
    ...
    # TODO[zsy]: add metrics 'reduce' in distributed training
    # Train & validation
    for epoch in range(args.max_epoch):
        ...
        train_bce_loss = torch.tensor(0, device=device, requires_grad=False)
        train_batch_num = torch.tensor(0, device=device, requires_grad=False)

        for batch_idx, (src, le, tgt, label, tgt_len) in enumerate(train_dataloader):
            src = src.to(device)
            le = le.to(device)
            tgt = tgt.to(device)
            ...
            optimizer.zero_grad()
            out_pos, out_stop = model(src=src, LE=le, tgt=tgt, tgt_key_padding_mask=tgt_len)
            # label_stop = torch.zeros_like(out_stop).to(device)
            # label_stop[torch.arange(label_stop.shape[0]), (tgt_len-1).squeeze(-1).long()] = 1  # 最后一位标1，即下一步不再预测

            time_range = torch.arange(out_stop.shape[1]).unsqueeze(0).to(device)  # (1, T)
            label_stop = (time_range >= (tgt_len-1)).float()  # 最后一位以及后续所有padding都标1
            mask_stop = (time_range < tgt_len)  # (32, 128)
            mask_pos = mask_stop.unsqueeze(2).unsqueeze(3).expand(-1, -1, out_pos.shape[2], out_pos.shape[3])

            loss_batch, mse_loss, bce_loss = loss_criterion(out_pos, label, out_stop, label_stop, mask_pos)
            train_loss = train_loss + loss_batch
            train_mse_loss = train_mse_loss + mse_loss
            train_bce_loss = train_bce_loss + bce_loss
        ...
        with torch.no_grad():
            ...
            val_bce_loss = torch.tensor(0, device=device, requires_grad=False)
            ...
            for batch_idx, (src, le, tgt, label, tgt_len) in enumerate(val_dataloader):
                src = src.to(device)
                le = le.to(device)
                tgt = tgt.to(device)
                label = label.to(device)
                ...
                # mask
                time_range = torch.arange(out_stop.shape[1]).unsqueeze(0).to(device)  # (1, 128)
                label_stop = (time_range >= tgt_len-1).float()  # 最后一位以及后续所有padding都标1
                mask_stop = (time_range < tgt_len)  # (32, 128)
                mask_pos = mask_stop.unsqueeze(2).unsqueeze(3).expand(-1, -1, out_pos.shape[2], out_pos.shape[3])
            ...
            val_bce_loss = reduce_mean(val_bce_loss, world_size)
            for key in metrics:
            ...
            if dist.get_rank() == 0:
                ...
                log_string(log, f'train bce loss: {train_bce_loss:.4f}, val bce loss: {val_bce_loss:.4f}')
