...
from utils.instruct_following import generate_counter_factual_instruct, check_consistency_batch
...
def reduce_mean(tensor, world_size):
    ...
    try:
        ...
        # Return original tensor if reduce fails
        return tensor
...
def train_dist(model, args, loss_criterion, optimizer, scheduler):
    ...
    for epoch in range(args.max_epoch):
        ...
        for batch_idx, (X, Y, TE_X, TE_Y, lang_list, _, _) in enumerate(train_dataloader):
            ...
            TE_Y = TE_Y.to(device)
            optimizer.zero_grad()
            output = model(X, TE_X, TE_Y, lang_list)
            if isinstance(output, tuple):
                out_pos, mu, logvar = output
                # Check for NaN or infinite values
                if torch.isnan(mu).any() or torch.isnan(logvar).any():
                    raise AssertionError(f"Rank {dist.get_rank()}: Warning - NaN detected in mu or logvar")
                total_loss, recon_loss, kl_loss = loss_criterion(out_pos, Y, mu, logvar)
            ...
            if torch.isnan(loss_batch):
                raise AssertionError(f"Rank {dist.get_rank()}: Warning - NaN loss detected, skipping batch")
        ...
        with torch.no_grad():
            ...
            for batch_idx, (X, Y, TE_X, TE_Y, lang_list, name_list, role_list) in enumerate(val_dataloader):
                X = X.to(device)
                Y = Y.to(device)
                # X = (X-args.mean) / args.std
                TE_X = TE_X.to(device)
