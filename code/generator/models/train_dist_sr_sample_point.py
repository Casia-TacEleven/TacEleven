...
def train_dist(model, args, loss_criterion, optimizer, scheduler):
    ...
    train_dataloader = DataLoader(train_dataset,
                                  ...
                                  collate_fn=lambda batch: collate_fn_self_regression_sample_point(batch, language_embedding, n_hist=args.n_hist, n_pred=args.n_pred))
    ...
    for epoch in range(args.max_epoch):
        ...
        for batch_idx, (src, le, tgt, label, tgt_len) in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            out_pos, out_stop = model(src=src, LE=le, tgt=tgt)
            loss_batch = loss_criterion(out_pos, label)
        ...
        with torch.no_grad():
            ...
            for batch_idx, (src, le, tgt, label, tgt_len) in enumerate(val_dataloader):
                ...
                metrics_outcome = calculate_metrics2(out_pos, label)
                for key in metrics:
